...
var getNewEmailFromDb = function (oldEmail, storage) {
    return new Promise(function (fulfill, reject) {
        var response = {"newEmail": ""};
        storage.emailChange.find({"oldEmail": oldEmail}, function (error, emailChange) {
            if (!utils.isEmpty(emailChange)) {
                response =  {"newEmail": emailChange[0].newEmail};
                fulfill(response);
...
// Insert user email in the message sent
...
var customisedControlledBroadcast = function(broadcastStartTime, action) {
    //logMessage("controlledBroadcast: " + action);
    var currentJob = job[broadcastStartTime];
    //logMessage(currentJob);
    if ((currentJob.items.length > 0) && (currentJob.waitTill <= utils.getTime())) {
        ...
        limiter.removeTokens(1, function(err, remainingRequests) {
            if (remainingRequests >= 1) {
                var user = currentJob.items.shift();
                sparkUtils.isUserActive(user).then(function (response) {
                    if (response == "active" && action == "notifyemailchange") {
                        getNewEmailFromDb(user, controller.storage).then(function (response) {
                            var message = currentJob.message.replace("$email", response.newEmail);
                            sendMessage(broadcastStartTime, message, user, false);
                        });

                    } else {
                        console.log("User" + user + "is not active");
                    }
...
/*
Notify Each User about a change personally
 */
SparkBroadcast.prototype.notifyEmailChange = function(bot, message, isTest, constructResponse) {
    var userList = [];
    var welcomeTxt = "";
    var admin_command = false;
    logMessage(message.match[0]);
    ...
    welcomeTxt = botResponse.getResponse('alert_email_change');
    ...
    buildUserList().then(function(userList) {
        ...
        if (!utils.isEmpty(userList)) {
            ...
            logMessage("%s: Starting Subscribe", broadcastStartTime);
            job[broadcastStartTime].timeout = setInterval(customisedControlledBroadcast, (1000 / process.env.bot_messagesPerSecond), broadcastStartTime, "notifyemailchange");
        }
    });
...
/*/
SparkBroadcast.prototype.publish = function(bot, message, broadcastTo, isTest, constructResponse) {
    ...
    logMessage("This is the message being posted: %s", content);
    var broadcastStartTime = 0;
